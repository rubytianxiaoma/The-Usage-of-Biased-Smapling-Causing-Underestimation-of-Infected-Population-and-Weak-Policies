...
selected_table <- data.frame(var = c('date',  'total cases','new cases','r','location', 'C(t)', 'C0','w0','s0'), describ = c('Date that the case was reported to the local public health unit(PHU)', 'Cumulative COVID-19 cases', 'Daily reported cases released by Public Health Ontario','Reproduction number: an estimate of the average infections that a COVID patient results', 'location','the cumulative number of confirmed cases at time t in days','initial number of confirmed cases at the time when the count starts','citizen’s willingness to get tested(if symptoms developed)','the percentage of symptomatic infected population, equals to 0.7 in the model'))
...
reasons <-c("If I were to experience symptoms","Had/thought been in contact with cases","Asymptomatic but worrying about others","No plan to get tested")
...
Figure 2 showed the varying reproduction number in Ontario, Canada [CCM, 2020] and Washington, U.S.[The COVID Tracking Project, 2020] at four time stages. However, the reported r is highly biased since the exponential growth indicates an infected person is able to infect more than 1 person. Reproduction number highly depends on other variables, such as time, country, strengths of government policy, even the person who estimates it. 
...
At this time, the term r^t in the model equals to 1 since t=0 when counting on a daily basis.The model is shown below:
